...
class PistolTeamData(models.Model):
...
def register_view(request):
  ...
  if request.method == 'POST':
    my_registration, created = PistolTeamData.objects.get_or_create(
  ...
  all_registrations = PistolTeamData.objects.filter(team=request.team)
  return render(request, 'hunt/special_puzzles/events/pistol.html', {
      'data': [{
          'name': reg.name,
